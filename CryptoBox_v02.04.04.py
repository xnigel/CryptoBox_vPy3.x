...
import secrets
import base64
import tempfile # Added for icon handling
...
def set_window_icon(root):
    try:
        ...
        with tempfile.NamedTemporaryFile(suffix=".png", delete=False) as temp_icon_file:
            temp_icon_file.write(icon_data)
            temp_path = temp_icon_file.name
        
        # Set the window icon from the temporary file
        root.iconphoto(True, PhotoImage(file=temp_path))
...
abt_msg = '''
The CryptoBox is a cryptographic calculator developed internally.
...
class CryptoBox(Tk):
    ...
    # Nigel: I forgot why I added this function. 
    # Leave this functin for now and ignore it. (2025.09.05)
    def input_file():
        global input_filename
        input_filename = askopenfilename(defaultextension='.txt') # type: ignore
        if input_filename == '':
            input_filename = None
        else:
            root.title('Key FileName:' + os.path.basename(input_filename))
            textPad.delete(1.0, END) # type: ignore
            f = open(input_filename, 'r')
            textPad.insert(1.0, f.read()) # type: ignore
            f.close()
...
if __name__ == "__main__":
    set_window_icon(root) # Call the icon function here.
    update_timeText()
    app = CryptoBox()
    root.mainloop()